        ...
        Alert.alert("ADVERTENCIA","Confirme Para Actualizar Sus Datos!",[ { 
            text: "Si", onPress: async () => {     
               ...
               if(nombre.length>=3){
                ...
                await axios.post('http://192.168.43.23:4005/api/usuarios/imagen?login='+usuario.login, formData, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                }).then().catch((err)=>{alert(err)})
              }

                await axios.put('http://192.168.43.23:4005/api/usuarios/editar?login='+usuario.login,
                {
                 nombre: nombre,
                 direccion: direccion,
                 telefono: telefono,
   ...
   <ScrollView>
    ...
    <View >
     ...
     (!imagen)?(<></>):(
      ...
      source={{uri:"http://192.168.43.23:4005/api/img/usuarios/"+usuario.imagen}} 
      resizeMode="contain"
