...
setCargaComentarios(cargaCometarios*2)
  ...
  return (
    ...
    <View style={{display:'flex',flexDirection:'column',justifyContent:'center',marginBottom:13}}>      
    {(!imagenn)?<></>:<Image source={{uri:"http://192.168.43.23:4005/api/img/usuarios/"+usuario.imagen}} resizeMode='cover' style={[{alignSelf:'center',height: 170,width:135}]}  />}
    <View>
    <Text  style={[Estilos.etiquetas,{alignSelf:'center',marginLeft:"2%",fontWeight:'bold'}]}>{usuario.nombre}</Text>
     ...
     <FlatGrid
      ...
      renderItem={({ item,index }) => (          
        (index>=6)? <></>:( 
          ...
          <ImageBackground
            source={{uri:"http://192.168.43.23:4005/api/img/usuarios/"+item.imagen}}
            resizeMode="cover"
...
<></>
             ...
             <Text style={{color:"white",fontSize:26,marginLeft:'35%'}}>Valoracion</Text>
            ...
            : <Button onPress={EnviarComentario} color='darkcyan'  title='Enviar'/>}
                     ...
                     renderItem={({ item,index }) => (                 
                       (index>=cargaCometarios)? <></>:( 
                         ...
                         <View  key={index} style={[Estilos.itemContainer, {height:300,display:'flex',justifyContent:'space-around', borderBottomWidth:8,marginBottom:5,backgroundColor: 'silver' }]}>
                          ...
                          <Text style={[Estilos.itemName,{color:'#f14fff',fontWeight:'bold',textAlign:'center',fontSize:30,letterSpacing:1}]}>{item.titulo.toUpperCase()}</Text>                          
                          <ScrollView>
              ...
              us.nombre=item.Usuario.nombre;
              us.imagen=item.Usuario.imagen;
              us.correo=item.Usuario.correo;
              us.user=item.Usuario.login; 
              us.dni=item.Usuario.dni;
              if(sesionIniciada){
              if(item.Usuario.login!=usuario.login){
              ...
              navigation.navigate("Perfiles")
                          ...
                          <Image source={{uri:"http://192.168.43.23:4005/api/img/usuarios/"+item.Usuario.imagen}} resizeMode='stretch' style={[{alignSelf:'center',height: 45,width:40,borderRadius:400/2}]}  />
                          
                          <Text style={[Estilos.itemName,{color:'#f15fff'}]}>{item.Usuario.login}</Text>

                          </TouchableOpacity>

         {(item.valoracion>=8)? <Text style={[Estilos.itemName,{color:'green'}]}>{item.valoracion}/10</Text>    : (item.valoracion>4)?<Text style={[Estilos.itemName,{color:'#f15fff'}]}>{item.valoracion}/10</Text>:<Text style={[Estilos.itemName,{color:'red'}]}>{item.valoracion}/10</Text> }

                          <Text style={[Estilos.itemName,{color:'#f15fff'}]}>{item.createdAt.substring(0,10)}</Text>
